...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("You canot put yourself in check");
		}
	...
	public ChessPiece replacePrometedPiece(String type) {
		...
		if(!type.equals("B") && !type.equals("B") && !type.equals("R") && !type.equals("Q")) {
			return promoted;
		}
	...
	private void validateSourcePosition(Position position) {
		if (!board.thereIsAPiece(position)) {
			throw new ChessException("There is no piece on source origin");
		}
		if (currentPlayer != ((ChessPiece) board.piece(position)).getColor()) {
			throw new ChessException("The chosen piece is not yours");
		}
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is no possible movements");
		}
	}

	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("The chosen piece canot move to target position");
		}
	}
